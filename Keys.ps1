...
# ---- FIRST FILTER: Keep everything from "Keys added:" onward ----
$index = $lines | Select-String -Pattern "Keys added:" | Select-Object -First 1 | ForEach-Object { $_.LineNumber - 1 }
...
# ---- SECOND FILTER: Remove everything after "Values deleted:" ----
$lines = Get-Content -Path $OutputFile

$cutIndex = $null
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match [regex]::Escape('Values deleted:')) {
        $cutIndex = $i
        break
    }
}
